...
class AppStore extends Component {
  state = {activeTabId: 'SOCIAL', searchInputValue: ''}
  ...
  onChanged = event => {
    ...
    this.setState({searchInputValue: typedValue})
  }

  render() {
    const {activeTabId, searchInputValue} = this.state
    ...
    const finalListApps = initialItems.filter(eachSearch =>
      eachSearch.appName.toLowerCase().includes(searchInputValue),
    )
          ...
          <ul className="ulTabsContainer">
            {tabsList.map(eachTabItem => (
              <TabItem
                ...
                generateAppItems={this.generateAppItems}
